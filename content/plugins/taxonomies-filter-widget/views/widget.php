...
<?php
	...
	echo '<li class="submit-button">';
	if( $display_reset_button && !empty($reset_button_label) ){
		echo '<a class="reset_button orange-box" href="'. esc_url( home_url( "/?post_type=$tfw_post_type") ).'" >'.$reset_button_label.'</a>';
	} // end if

	if ($auto_submit) {   echo '<noscript>';  } 
		echo '<input type="submit" value="'.$submit.'" class="orange-box" />';
	if ($auto_submit) {   echo '</noscript>';  } 
