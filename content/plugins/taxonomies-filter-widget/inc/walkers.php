	...
	function start_lvl( &$output, $depth = 0, $args = array() ) { $output .= "<li><ul class='children'>"; }	
  	...
  	public function start_el(&$output, $term, $depth, $args){
	    ...
	    if (!(!$counter && $hide_empty)){
		    ob_start(); ?>   
		    <li>
		    	<label for="<?php echo $name."-".$term->slug; ?>">
		      		<input type="checkbox" <?php checked(in_array($term->slug, explode($tfw_options['multiple_relation'],$selected))); ?> name="<?php echo $name ?>" id="<?php echo $name."-".$term->slug; ?>"  value="<?php echo $term->slug; ?>" /> <?php echo $term->name; 
		      		echo ($show_count == "none") ? '' : '&nbsp;<span class="tax_count">('.  $counter .')</span>'; 
			?>	</label>  
			</li>     
			<?php 
		    $output .= ob_get_clean();
